...
description: Agent QC profile mapping for a Codex-style runtime CLI, TUI, protocol, and release stack.
---
...
- static: `cargo fmt`, `cargo clippy`, dependency/policy checks, Bazel parity.
- unit/runtime: targeted crate tests and `cargo nextest run --no-fail-fast`.
- contract/protocol: MCP fixtures, SSE fixtures, app-server protocol schemas, SDK/API tests.
