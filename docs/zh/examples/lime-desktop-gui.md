...
description: 包含 Tauri bridge、WebView panel、browser runtime 的 desktop GUI 的 Agent QC profile 映射。
---
...
- static/local：`npm run verify:local` 与定向 package/Rust check。
- contract/protocol：frontend call、Rust registration、catalog、DevBridge、mock 之间的 command contract。
- desktop-gui：shell start/reuse、DevBridge health、workspace readiness、screenshot/trace、console state。
- webui：settings、skills、resources、MCP、artifacts、chat shell 等 WebView panel 的 component/browser test。
- browser-automation：browser runtime/site adapter smoke 与真实 product flow 的 Playwright continuation。
