---
title: Claude Code TUI runtime 快照示例
description: metadata 不完整的 Claude Code 风格本地 TUI/runtime 快照的 Agent QC profile 映射。
---

# Claude Code TUI runtime 快照示例

本示例基于 `/Users/coso/Documents/dev/js/claudecode` 的本地快照。该快照不包含 package 或 workflow metadata，所以示例只声明 interface-surface observations。
...
- contract/protocol：remote session manager、WebSocket/control schema、`can_use_tool` permission flow。
- ui-interaction：command view、synthetic permission row、reconnect/cancel state 的 Ink TUI snapshot。
- runtime-e2e：remote cancellation、interrupt、reconnect、stale-state prevention。
- sdk-api：保留 event type 与 tool-use id 的 stream adapter fixture。
- review：因为本地快照没有 CI/test runner metadata，必须显式记录限制。
