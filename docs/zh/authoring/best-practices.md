...
这页是 Agent QC 计划的编写检查表。它吸收 Agent UI 的 runtime-first 思路、Agent Skills 的渐进披露文档风格，以及 Codex、Claude Code 本地快照、OpenClaw、Hermes Agent、Lime 中观察到的测试组织方式。

Agent QC 是标准，不是 Lime 专用清单。Lime 只是桌面 GUI 案例。
...
如果缺少 bridge/runtime 证据，不要因为 UI 看起来健康就写 `passed`。这是 Lime GUI 的教训，也适用于所有 Agent UI/TUI/WebUI 项目。
...
| 本地案例 | Codex、Claude Code 本地快照、OpenClaw、Hermes、Lime |
...
| 问题 | 必须回答 |
| --- | --- |
| Lime 是否只是案例？ | 是；profiles 适用于所有 Agent 项目类型。 |
