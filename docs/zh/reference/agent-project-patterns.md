---
title: Agent 项目测试模式
description: 来自 Codex、OpenClaw、ClaudeCode 快照、Hermes Agent 和 Lime 的案例模式。
---
...
- fixture 包括 fake model server、SSE fixture、测试用 stdio/http server 和期望 patch output。

Agent QC profile 映射：`agent-runtime-cli`、`agent-tool-mcp-gateway`、`agent-sdk-api`、`agent-distribution-release`。
...
Agent QC profile 映射：`multi-channel-agent-gateway`、`agent-tool-mcp-gateway`、`agent-skills-plugins`、`agent-distribution-release`、`agent-evals-quality`。

## ClaudeCode 风格 TUI/runtime 快照
...
- 源码表面仍显示 TUI/runtime 风险：shell interaction、remote session permission request、tool-use message、SDK adapter 和可注入 query dependency。
- Agent QC 只能把它当成 interface-surface 参考，不能据此推断上游项目完整测试策略。

Agent QC profile 映射：`agent-runtime-cli`、`agent-ui-tui-desktop`、`agent-sdk-api`、`agent-tool-mcp-gateway`。
...
- CI 包含 pytest、独立 e2e、ruff/ty lint、uv lock check、OSV scanner、Docker build/smoke、docs-site check 和 skills index job。

Agent QC profile 映射：`background-agent-scheduler`、`multi-channel-agent-gateway`、`agent-ui-tui-desktop`、`agent-distribution-release`、`agent-skills-plugins`。
...
- 必需门禁包括 `verify:local`、`test:contracts`、`verify:gui-smoke`，用户可见行为变更时继续 Playwright 验证。
