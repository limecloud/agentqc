---
title: 明星项目测试体系
description: Codex、Claude Code 本地快照、OpenClaw、Hermes Agent 和 Lime 的测试体系案例。
---
...
| Lime | runtime command 与 bridge contract | 非主 surface | WebView/component panel | 强 Tauri desktop GUI smoke | browser runtime/site adapter smoke 与 Playwright continuation | 非主 gateway | product/evidence report | app version、Tauri/Rust/package check |
...
## Lime：desktop GUI + Tauri bridge + browser runtime

本地来源：`/Users/coso/Documents/dev/ai/aiclientproxy/lime`。

### 产品形态

Lime 是 desktop GUI Agent 产品：

- Tauri shell 与 frontend WebView；
- DevBridge 与 browser fallback mock layer；
- workspace 和 resource manager UI；
- browser runtime panel 与 site adapter；
- skills/capability drafts、MCP panel、provider settings、memory、artifacts、chat shell；
- Rust command surface 与 command contract catalog。

### 测试如何组织

| 层级 | 具体信号 | Agent QC 解释 |
| --- | --- | --- |
| Unified local gate | `npm run verify:local`、`verify:local:full` | 聚合本地 readiness |
...
| Component/UI tests | workspace、browser runtime、settings、skills、MCP、resources、artifacts、chat shell 的 `*.test.tsx` | WebView component evidence |
| Product E2E | product/knowledge E2E 与 Playwright MCP continuation guidance | user-visible flow evidence |
| Rust/Tauri | targeted Cargo tests、Rust lint/test scripts | backend command evidence |
| Version/release | package/Tauri/Cargo 文件间的 app version consistency check | distribution consistency |

### GUI 细节

Lime 的规则很明确：GUI 产品可交付性不能只由 `lint`、`typecheck` 或 unit test 证明。

Lime 风格 desktop pass 应证明：

- shell 通过项目入口启动或复用；
- 判断页面前 DevBridge 已健康；
- default workspace 已准备；
- command contract 在 frontend call、Rust registration、governance catalog、DevBridge、mock 之间同步；
- browser runtime/site adapter panel 不用 mock 掩盖 bridge failure；
- 用户可见变化有稳定回归测试或 Playwright evidence；
- GUI smoke 失败时保留 console state 与 screenshot/trace。

Agent QC 规则：desktop GUI gate 必须说明 shell、bridge、workspace、mock/real backend、screenshot/trace、console state 和 OS。
