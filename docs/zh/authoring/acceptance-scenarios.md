...
1. 从真实 Lime 失败或高风险流程中选择 frozen dataset 和 task set。
2. Baseline 与 candidate config 只允许一个变量不同。
3. 每个 trial 保存 trajectory、runtime transcript、reward details、artifacts、timeout/cost metrics 和 cleanup evidence。
4. Aggregate comparison 报告 reward delta、timeout rate、evidence completeness 和 P0 QC regression count。
5. 即使 reward 改善，只要 required QC gates 回退，就阻断 candidate promotion。

通过条件：improvement claim 可复现、可归因，并且仍兼容 required Agent QC gates。

证据：dataset/task version、baseline/candidate configs、trial trajectories、reward details、comparison summary、Agent QC report refs。
