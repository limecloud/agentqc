...
| `kind` | Yes | 例如 `command-log`、`test-report`、`protocol-transcript`、`surface-artifact`、`release-artifact`、`eval-artifact`、`benchmark-artifact`、`trajectory`、`reward-details`、`review-note`、`qcloop-run`。 |
...
- 如果声明 improvement，benchmark evidence 包含 frozen dataset、trial trajectories、reward details 和 baseline/candidate configs；
